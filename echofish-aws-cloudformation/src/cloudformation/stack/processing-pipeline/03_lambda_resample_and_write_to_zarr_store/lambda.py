...
def main(
        ...
        file_name: str='D20070712-T202050.raw'
) -> None:
    #
    #
    ...
    start_file_index = df.index[df['FILENAME'] == file_name][0]
    ...
    store = s3fs.S3Map(
        root=f's3://{INPUT_BUCKET}/data/processed/{ship_name}/{cruise_name}/{sensor_name}/{file_name}.zarr',
        s3=s3,
        check=False
    )
